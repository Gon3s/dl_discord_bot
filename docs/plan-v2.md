...
Le projet actuel est un bot Discord minimaliste (3 fichiers Python) permettant de chercher et télécharger des films/séries/mangas depuis Wawacity via AllDebrid. L'interface Discord est peu ergonomique, le code est monolithique et non extensible.

La v2 transforme ce bot en une **application trois tiers** :
- **FastAPI backend** — couche logique partagée (scraping, debrid, téléchargement, BDD)
- **Angular + Tailwind 4 frontend** — interface web principale, bien plus utilisable que le bot
- **Discord bot allégé** — thin client qui appelle le backend, garde sa valeur pour les usages rapides
...
class BaseScraper(ABC):
    ...
    @abstractmethod
    async def get_download_links(self, url) -> list[ProviderLink]: ...
```

- `wawacity.py` — migration de `parser.py` (Selenium + BS4)
- `darkiworld.py` — stub avec `NotImplementedError`
- `search_service.py` instancie le bon scraper selon `source` param
...
8. **Docker** — `docker-compose.yml` pour tout lancer
...
1. `docker-compose up` → backend sur `:8000`, frontend sur `:4200`
